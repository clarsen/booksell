...
class Migration:
    ...
    def forwards(self, orm):
        ...
        # Adding model 'HalfOffer'
        db.create_table('books_halfoffer', (
            ('id', orm['books.halfoffer:id']),
            ('book', orm['books.halfoffer:book']),
            ('listing_id', orm['books.halfoffer:listing_id']),
            ('price', orm['books.halfoffer:price']),
            ('seller_id', orm['books.halfoffer:seller_id']),
            ('condition', orm['books.halfoffer:condition']),
            ('created', orm['books.halfoffer:created']),
            ('modified', orm['books.halfoffer:modified']),
            ('active', orm['books.halfoffer:active']),
        ))
        db.send_create_signal('books', ['HalfOffer'])
        
        # Changing field 'Book.condition'
        # (to signature: django.db.models.fields.CharField(max_length=255, null=True, blank=True))
        db.alter_column('books_book', 'condition', orm['books.book:condition'])
        
    
    
    def backwards(self, orm):
        
        # Deleting model 'HalfOffer'
        db.delete_table('books_halfoffer')
